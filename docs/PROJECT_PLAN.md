...
- First-run flow: `1 选工具 / 2 选任务 / 3 点生命场`, actionable recommended-tool chip, canvas coachmark, and local feedback pulses.
...
- Submission: `scripts/package_submission.py` assembles the native-first judge folder with Windows launchers, fallback materials, package manifest, and optional school/group/student metadata.
...
3. Run `python3 scripts/package_submission.py --out dist/peterMath_windows_submission`. For final delivery, add `--official-name`, `--school`, `--group`, and `--student`.
...
- `1 选工具` has a visible control in the mission panel, not just text.
